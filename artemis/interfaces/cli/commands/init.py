...
def init_command() -> None:
    """Interactive quickstart wizard to configure a model provider and device."""
    console = Console()
    console.print()
    console.print(
        Panel(
    ...
    console.print("  [1] Signed-in Codex client (Recommended - no API key)")
    console.print("  [2] Google Gemini")
    console.print("  [3] OpenAI API")
    console.print("  [4] Anthropic")
    ...
    # 2. Verify the local client, or collect the selected cloud API key.
    api_key = ""
    if env_key is None:
        ...
        if not available:
            if shutil.which("codex") is None:
                if platform.system() == "Windows":
                    remedy = (
                        "Run [bold cyan].\\scripts\\install_deps.ps1[/bold cyan], or install Codex with:\n"
                        "[cyan]$env:CODEX_NON_INTERACTIVE=1; irm "
                        "https://chatgpt.com/codex/install.ps1 | iex[/cyan]"
                    )
                else:
                    remedy = (
                        "Run [bold cyan]bash scripts/install_deps.sh[/bold cyan], or install Codex with:\n"
                        "[cyan]curl -fsSL https://chatgpt.com/codex/install.sh | sh[/cyan]"
                    )
            else:
                remedy = (
                    "Run [bold cyan]codex login[/bold cyan]. For a terminal without a browser, "
                    "use [bold cyan]codex login --device-auth[/bold cyan]."
                )
            console.print(
                "\n[bold red]Codex is not ready.[/bold red] "
                f"{detail}\n{remedy}\nThen run this wizard again."
            )
            raise SystemExit(1)
        console.print("\n[bold green]✔ Codex client is signed in with ChatGPT.[/bold green]")
    else:
